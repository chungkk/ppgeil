...
export const useShadowingLogic = () => {
  ...
  // Auto-translate transcript based on user's native language setting
  useEffect(() => {
    // Get user's native language from settings, default to 'vi' if not set
    const targetLang = user?.nativeLanguage || 'vi';
    
    // Skip if: no data, already translating, or German (same as source)
    if (
    ...
    // Skip if already translated to the same language
    if (currentTranslationLang === targetLang && transcriptData[0]?.translation) {
      return;
    }

    const translateTranscript = async () => {
      // Check cache first
      const cachedTranslations = getSentenceTranslationCache(lessonId, targetLang);
      if (cachedTranslations && cachedTranslations.length === transcriptData.length) {
        // Apply cached translations
        setTranscriptData(prev => prev.map((item, idx) => ({
          ...item,
          translation: cachedTranslations[idx] || item.translation
        })));
      ...
      // Clear existing translations first when language changes
      if (currentTranslationLang && currentTranslationLang !== targetLang) {
      ...
      for (let i = 0; i < transcriptData.length; i += batchSize) {
        ...
        const batchPromises = batch.map(item => translateSentence(item.text, targetLang));
        ...
        setTranscriptData(prev => prev.map((item, idx) => ({
          ...item,
          translation: translations[idx] || item.translation
        })));
        
        // Small delay between batches to avoid rate limiting
        if (i + batchSize < transcriptData.length) {
          await new Promise(resolve => setTimeout(resolve, 100));
        }
      }
      
      // Cache translations
      setSentenceTranslationCache(lessonId, targetLang, translations);
      setCurrentTranslationLang(targetLang);
    ...
    translateTranscript();
  }, [transcriptData.length, lessonId, user?.nativeLanguage, currentTranslationLang]);
