...
export const useShadowingLogic = () => {
  ...
  useEffect(() => {
    const checkMobile = () => {
      ...
      // Turn off translation by default on mobile (only on first load)
      if (isMobileView && !isClientReady) {
  ...
  const handleSentenceClick = useCallback((startTime, endTime, forceFromStart = false) => {
    ...
    if (isCurrentlyPlayingThisSentence && !forceFromStart) {
      if (isYouTube) {
        const player = youtubePlayerRef.current;
      ...
      if (!forceFromStart && pausedPositions[clickedIndex] && pausedPositions[clickedIndex] >= startTime && pausedPositions[clickedIndex] < endTime) {
  ...
  const handleWordClickForPopup = useCallback(async (word, event) => {
    ...
    if (isMobileView) {
      ...
      // If tooltip would go off top, show below the word
      if (tooltipTop - tooltipHeight < 10) {
        tooltipTop = rect.bottom + 10 + tooltipHeight;
      }
      ...
      if (tooltipLeft + halfWidth > window.innerWidth - 10) {
        tooltipLeft = window.innerWidth - halfWidth - 10;
      }
      ...
      setTooltipTranslation(translation || 'Đang tải...'); // Show cached translation or loading
      setTooltipPosition({ top: tooltipTop, left: tooltipLeft });
      setShowTooltip(true);

      // Fetch translation if not in cache - send context for better quality
      if (!translation) {
        try {
          ...
          if (response.ok) {
            ...
            setTooltipTranslation(translation || 'Không có bản dịch');
          }
        } catch (error) {
          console.error('Translation error:', error);
          setTooltipTranslation('Không có bản dịch');
        }
      }
    } else {
  ...
  }, [user, incrementStreak, resetStreak]);
