...
export const useShadowingLogic = () => {
  ...
  useEffect(() => {
    ...
    // Skip if: no data, already translating, German (same as source), or already has translations
    if (
      transcriptData.length === 0 || 
      translationInProgressRef.current ||
      targetLang === 'de' ||
      transcriptData[0]?.translation
    ) {
      return;
    }
    ...
    translateTranscript();
  }, [transcriptData.length, lessonId, user?.nativeLanguage]);
