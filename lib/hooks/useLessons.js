...
export function useLessons({ page = 1, limit = 15, difficulty = 'all', category = null } = {}) {
  ...
  // T060: Add category parameter support
  if (category) {
...
export function prefetchLessons({ page, limit = 15, difficulty = 'all', category = null }) {
  const queryParams = new URLSearchParams({
    page: page.toString(),
    limit: limit.toString()
  });
  ...
  // T060: Add category parameter support for prefetching
  if (category) {
    queryParams.set('category', category);
  }
