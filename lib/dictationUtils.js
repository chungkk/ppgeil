...
/**
 * Get adaptive size class name based on word length
 */
export const getWordSizeClassName = (wordLength) => {
  if (wordLength <= 3) return 'word-size-short';
  if (wordLength <= 6) return 'word-size-medium';
  if (wordLength <= 10) return 'word-size-long';
  return 'word-size-xlong';
};
...
export const renderCompletedSentenceWithWordBoxes = (sentence) => {
  ...
  return words.map((word, idx) => {
    ...
    // Get adaptive size class
    const sizeClass = getWordSizeClassName(pureWord.length);
    
    return `<span class="word-container">
      <span class="correct-word completed-word ${sizeClass}" onclick="window.handleWordClickForPopup && window.handleWordClickForPopup('${pureWord}', this)">${pureWord}</span>
